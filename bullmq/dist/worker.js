...
const worker = new bullmq_1.Worker('stagehand-tasks', async (job) => {
    ...
    try {
        ...
        await prisma.todo.update({
            ...
            data: {
                status: 'COMPLETED',
                resultData: result, // Cast to any for Prisma JSON field
...
async function processResearchAgent(data) {
...
async function processBrowserAutomation(data) {
...
async function processSearchAgent(data) {
...
async function processActionScout(data) {
...
async function processSummaryAgent(data) {
...
async function processPlexAgent(data) {
...
// Create individual todo summary
async function createIndividualTodoSummary(todo, result) {
    try {
        const aiWorkerUrl = process.env.AI_WORKER_URL || 'http://localhost:8787';
        const response = await fetch(`${aiWorkerUrl}/task`, {
            method: 'POST',
...
// Update scout progress
async function updateScoutProgress(scoutId) {
    try {
