...
app.post('/add-task', async (req, res) => {
    try {
        ...
        try {
            schemas_1.completeTaskSchema.parse(jobData);
        }
        catch (error) {
            return res.status(400).json({
        ...
        // Add job to queue
        const job = await (0, queue_1.addStagehandTask)(jobData);
...
app.get('/queue-status', async (req, res) => {
    try {
        const waiting = await queue_1.jobsQueue.getWaiting();
        const active = await queue_1.jobsQueue.getActive();
        const completed = await queue_1.jobsQueue.getCompleted();
        const failed = await queue_1.jobsQueue.getFailed();
        res.json({
...
app.get('/job/:jobId', async (req, res) => {
    try {
        const job = await queue_1.jobsQueue.getJob(req.params.jobId);
        if (!job) {
...
async function startServer() {
    ...
    const redisConnected = await (0, queue_1.testRedisConnection)();
    if (!redisConnected) {
